...
const LandingCard = () => {
  ...
  return (
    <div className={`relative z-20 w-full max-w-3xl mx-auto flex flex-col items-center transition-all duration-1000 ${isWarping ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
      <div className="bg-black/60 backdrop-blur-md border border-white/10 rounded-3xl p-8 md:p-12 shadow-2xl space-y-8 text-center relative overflow-hidden group">
        ...
        {/* Title with Glitch */}
        <div className="relative py-16 glitch-wrapper">
            <h1 
                className="text-6xl md:text-9xl font-bold text-white select-none tracking-tighter glitch"
                data-text="d3FRAG"
            >
                d3FRAG
            </h1>
            <div className="absolute bottom-7 left-0 right-0">
                <span className="text-xl md:text-2xl font-mono text-neon-green text-glow-green tracking-[0.5em] font-bold opacity-80">NETWORKS</span>
            </div>
        </div>
        ...
        {/* Data Grid */}
        <div className="grid grid-cols-1 gap-px bg-white/10 border border-white/10 rounded-lg overflow-hidden max-w-sm mx-auto text-left text-xs font-mono">
